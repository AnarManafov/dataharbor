...
def cmd_consolidate(args: Any) -> int:
    """Consolidate duplicate entries in a changelog."""
    ...
    # Consolidate entries and write back
    new_content = write_consolidated_changelog(content, sections)

    if args.dry_run:
        print("Dry run. Here's the consolidated changelog:")
        print(new_content)
    else:
        with open(changelog_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Successfully consolidated changelog at {changelog_path}")

    return 0


def cmd_extract(args: Any) -> int:
    """Extract content for a specific version from the changelog."""
    # Determine changelog path
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified changelog management tool")
    ...
    generate_parser = subparsers.add_parser(
        "generate",
        help="Generate a changelog for a specific range of commits")
    generate_parser.add_argument(
        "--version",
        help="Version to add to changelog (e.g., '1.2.3')")
    generate_parser.add_argument(
        "--from-tag",
        help="Starting tag for commits (default: latest tag)")
    generate_parser.add_argument(
        "--to-tag",
        default="HEAD",
        help="Ending reference (tag or commit) (default: HEAD)")
    generate_parser.add_argument(
        "--output",
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update an existing changelog with a new version")
    ...
    full_parser = subparsers.add_parser(
        "full",
        help="Generate a full changelog from all version tags")
    ...
    extract_parser = subparsers.add_parser(
        "extract",
        help="Extract content for a specific version from the changelog")
    ...
    extract_parser.add_argument(
        "--output",
        help="Path to the CHANGELOG.md file (default: CHANGELOG.md in repo root)")
    extract_parser.add_argument(
        "--output-file",
        required=True,
        help="File to write the extracted content to")
