 ...
 */
function main() {
  // CI workflows (version-tag-processor) pass explicit version env vars because the new
  // release tags do not exist yet when this script runs.  When those vars are present, use
  // them directly instead of falling back to git-describe which would find the previous tag.
  const envGlobal = process.env.GLOBAL_VERSION || null;
  const envFrontend = process.env.FRONTEND_VERSION || null;

  const globalVersionInfo = envGlobal
    ? { version: envGlobal, hasCommitsAfterTag: false, rawOutput: `env:${envGlobal}` }
    : getVersionInfo('v*');
  const frontendVersionInfo = envFrontend
    ? { version: envFrontend, hasCommitsAfterTag: false, rawOutput: `env:${envFrontend}` }
    : getVersionInfo('web/v*');
