	...
	<div class="browse-view">
		...
		<div class="file-browser-container">
			...
			<div class="file-table-container">
				...
				<FileTable ref="fileTableRef" :tableLoading="tableLoading" :filteredData="filteredData" :filters="filters"
					@selectDir="selectDir" @downloadFile="handleDownloadFile" @selectionChange="handleSelectionChange" />

				<!-- Show empty state overlay when no actual files exist, but still allow table to show parent dir -->
				<div v-if="!tableLoading && tableData.length === 0 && filteredData.length <= 1" class="empty-state-overlay">
			...
			<transition name="slide-up">
				<div v-if="selectedFiles.length > 0" class="batch-action-bar">
					<div class="batch-action-info">
						...
						<span>{{ selectedFiles.length }} file{{ selectedFiles.length > 1 ? 's' : '' }} selected
							({{ selectedFilesSize }})</span>
					</div>
...
// FileTable ref for clearing selection
...
const selectedFilesSize = computed(() => {
	...
	return filters.prettyBytes(total);
});
...
// Clear file selection when navigating to a different directory
...
/**
 * Handle batch download of selected files as tar.gz
 */
const handleBatchDownload = async () => {
	if (selectedFiles.value.length === 0) return;

	const fileNames = selectedFiles.value.map(f => f.name);
	const totalSize = selectedFiles.value.reduce((acc, f) => acc + (f.size || 0), 0);

	try {
		await ElMessageBox.confirm(
			`Download ${fileNames.length} file${fileNames.length > 1 ? 's' : ''} (${filters.prettyBytes(totalSize)}) as a .tar.gz archive?`,
			'Batch Download',
			{
				confirmButtonText: 'Download',
				cancelButtonText: 'Cancel',
