...
The upload feature relies on SciToken-based authorization at the XRootD
layer. The user's access token must carry scopes authorizing the target
path:

- `storage.create:/<path>` — required for every upload (session creation,
  chunk writes, rename to final path).
- `storage.modify:/<path>` — additionally required when the server is
  configured with `xrd.upload.allowOverwrite: true` **and** the user
  chooses the `overwrite` conflict action.
- `storage.read:/<path>` — required for existing browse/download behavior
  (unchanged).

Configure your IdP to include these scopes in the access tokens issued
for the DataHarbor client. See [UPLOAD.md](./UPLOAD.md) for the end-to-end
flow and [xrootd.md](./xrootd.md) for the server-side `libXrdAccSciTokens`
setup.
